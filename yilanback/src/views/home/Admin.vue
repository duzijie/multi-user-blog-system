<template>
  <div class="user">
    ...
    <!-- 增加弹出框 - 内置 新增和修改 表单 -->
    <el-dialog :title="textMap1" :visible.sync="dialogFormVisible">
      <el-form
        ref="dataForm"
        :rules="rules"
        :model="temp"
      ...
      >
        ...
        <el-form-item label="性别" prop="sex">
          <el-select v-model="temp.sex" class="sex" placeholder="Please select">
            <el-option
              v-for="item in sexTypeOptions"
              :key="item.key"
              :label="item.display_name"
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.axios
        .post("/api/blogsystem/user/super/searchAdminsByParams", this.listQuery)
        .then((res) => {
    ...
    addFilter: function () {
      ...
      this.$nextTick(() => {
        //<el-form ref="dataForm"  找到dialog重的Form 并 清空 校验提示
        this.$refs["dataForm"].clearValidate();
      });
    },
    createData: function () {
      this.dialogFormVisible = false;
      //this.temp.roles = "ROLE_ADMIN";
      console.info(this.temp);
      this.axios
        .post("/api/blogsystem/user/super/insertAdmin", this.temp)
        .then((res) => {
          console.info(res.data);
    ...
    //下调权限
    demotionPermissions: function (row) {
      console.info(row.id)
      row.roles = "ROLE_USER";
      this.axios
    ...
    //修改管理员状态
    handleModifyStatus: function (row) {
      console.info(row);
      this.axios
        .post("/api/blogsystem/user/super/switchIslockToAdmin", row)
        .then((res) => {
          row.islock = res.data.islock;
          if (res.data.code == 0) {
